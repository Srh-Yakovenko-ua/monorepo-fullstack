...
describe("Security devices API", () => {
  describe("GET /api/security/devices", () => {
    it("returns 401 when no refreshToken cookie is sent", async () => {
      const res = await request(server).get("/api/security/devices");
    ...
    it("returns 4 active devices after 4 logins with different user-agents", async () => {
      ...
      const chrome = request.agent(server);
      const safari = request.agent(server);
      const firefox = request.agent(server);
      const edge = request.agent(server);
  ...
  describe("DELETE /api/security/devices", () => {
    ...
    it("returns 401 when no refreshToken cookie is sent", async () => {
      const res = await request(server).delete("/api/security/devices").set("Origin", TEST_ORIGIN);
