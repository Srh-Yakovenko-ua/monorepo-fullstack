...
describe("Blogs API", () => {
  describe("GET /api/blogs", () => {
    ...
    it("returns paginator with one item after creating one", async () => {
      await request(server).post("/api/blogs").send(validBlog);
    ...
    it("respects pageNumber and pageSize", async () => {
      ...
      const res = await request(server).get("/api/blogs?pageNumber=2&pageSize=3");
    ...
    it("sorts by name asc", async () => {
      ...
      const res = await request(server).get("/api/blogs?sortBy=name&sortDirection=asc");
    ...
    it("sorts by name desc", async () => {
      ...
      const res = await request(server).get("/api/blogs?sortBy=name&sortDirection=desc");
    ...
    it("filters by searchNameTerm case-insensitively", async () => {
      ...
      const res = await request(server).get("/api/blogs?searchNameTerm=tech");
    ...
    it("empty searchNameTerm returns all blogs", async () => {
      ...
      const res = await request(server).get("/api/blogs?searchNameTerm=");
    ...
    it("treats regex metacharacters in searchNameTerm as literal text", async () => {
      ...
      const wildcardRes = await request(server).get("/api/blogs?searchNameTerm=*");
      expect(wildcardRes.status).toBe(200);
      expect(wildcardRes.body.items).toHaveLength(0);

      const charClassRes = await request(server).get(
    ...
    it("rejects searchNameTerm longer than 100 characters", async () => {
      ...
      const res = await request(server).get(`/api/blogs?searchNameTerm=${longTerm}`);
      expect(res.status).toBe(400);
    });

    it("returns 400 for invalid sortBy field", async () => {
      const res = await request(server).get("/api/blogs?sortBy=invalid");
  ...
  describe("POST /api/blogs", () => {
    it("creates a blog and returns 201 with view model", async () => {
      const res = await request(server).post("/api/blogs").send(validBlog);
    ...
    it("returns 400 when websiteUrl is missing", async () => {
      const res = await request(server)
        .post("/api/blogs")
        .send({ description: "desc", name: "Blog" });
  ...
  describe("GET /api/blogs/:id", () => {
    it("returns the blog by id", async () => {
      const created = await request(server).post("/api/blogs").send(validBlog);
    ...
    it("returns 404 for unknown id", async () => {
      const res = await request(server).get("/api/blogs/000000000000000000000000");
  ...
  describe("PUT /api/blogs/:id", () => {
    ...
    it("returns 404 for unknown id", async () => {
      const res = await request(server).put("/api/blogs/000000000000000000000000").send(validBlog);
  ...
  describe("DELETE /api/blogs/:id", () => {
    it("deletes the blog and returns 204", async () => {
      ...
      const res = await request(server).delete(`/api/blogs/${id}`);

      expect(res.status).toBe(204);
    });

    it("returns 404 for unknown id", async () => {
      const res = await request(server).delete("/api/blogs/000000000000000000000000");
  ...
  describe("GET /api/blogs/:id/posts", () => {
    it("returns empty paginator for blog with no posts", async () => {
      const blog = await request(server).post("/api/blogs").send(validBlog);
      const id: string = blog.body.id;

      const res = await request(server).get(`/api/blogs/${id}/posts`);
    ...
    it("returns paginator with posts belonging to the blog", async () => {
      ...
      await request(server).post(`/api/blogs/${id}/posts`).send(validPost);
      await request(server)
    ...
    it("returns 404 for unknown blog id", async () => {
      const res = await request(server).get("/api/blogs/000000000000000000000000/posts");
    ...
    it("returns 400 for invalid sortBy field", async () => {
      ...
      const res = await request(server).get(`/api/blogs/${id}/posts?sortBy=invalid`);
    ...
    it("respects pageNumber and pageSize", async () => {
      ...
      const res = await request(server).get(`/api/blogs/${id}/posts?pageNumber=2&pageSize=3`);
  ...
  describe("POST /api/blogs/:id/posts", () => {
    it("creates a post for a blog and returns 201", async () => {
      ...
      const res = await request(server).post(`/api/blogs/${id}/posts`).send(validPost);
